...
try:
    ...
    print(f"[CUDA DETECTION] getCudaEnabledDeviceCount() = {count}")
    if count > 0:
        cv2.cuda.setDevice(0)

        # Forcer l'initialisation CUDA avec un test simple
        print(f"[CUDA DETECTION] Testing CUDA initialization...")
        test_mat = cv2.cuda_GpuMat()
        test_array = np.ones((10, 10), dtype=np.uint8) * 100
        test_mat.upload(test_array)
        print(f"[CUDA DETECTION] Test upload: type={test_mat.type()}, size={test_mat.size()}, depth={test_mat.depth()}")
        test_back = test_mat.download()
        print(f"[CUDA DETECTION] Test download: dtype={test_back.dtype}, shape={test_back.shape}")

        # Test cv2.cuda.threshold directement
        print(f"[CUDA DETECTION] Testing cv2.cuda.threshold...")
        try:
            _, test_thresh = cv2.cuda.threshold(test_mat, 50, 255, cv2.THRESH_BINARY)
            test_thresh_cpu = test_thresh.download()
            print(f"[CUDA DETECTION] Threshold test OK: output shape={test_thresh_cpu.shape}")
        except Exception as e:
            print(f"[CUDA DETECTION] Threshold test FAILED: {e}")
            raise

        USE_CUDA = True
        print(f"[CUDA DETECTION] USE_CUDA = {USE_CUDA}")
    ...
    print(f"[CUDA DETECTION] AttributeError: {e}")
    if _SHOULD_PRINT_GPU_INFO:
        print("⚠️  OpenCV compilé SANS support CUDA - Mode CPU uniquement")
    USE_CUDA = False
except Exception as e:
    # Autre erreur
    print(f"[CUDA DETECTION] Exception: {type(e).__name__}: {e}")
    USE_CUDA = False

print(f"[CUDA DETECTION] FINAL USE_CUDA = {USE_CUDA}")
...
def ensure_gpu(image):
    ...
    if USE_CUDA:
        if isinstance(image, cv2.cuda_GpuMat):
            print(f"[ensure_gpu] Already GpuMat, type={image.type()}")
            return image
        if isinstance(image, np.ndarray):
            print(f"[ensure_gpu] CPU->GPU: dtype={image.dtype}, shape={image.shape}")
        gpu_mat = cv2.cuda_GpuMat()
        gpu_mat.upload(image)
        print(f"[ensure_gpu] Uploaded, GPU type={gpu_mat.type()}")
        return gpu_mat
    return image
...
def remove_lines_param(gray_image, h_size, v_size, dilate_iter):
    ...
    if USE_CUDA:
        # Pipeline 100% GPU
        print(f"[remove_lines] Input type: {type(gray_image)}")
        if isinstance(gray_image, cv2.cuda_GpuMat):
            print(f"[remove_lines] Input GpuMat type={gray_image.type()}")
        elif isinstance(gray_image, np.ndarray):
            print(f"[remove_lines] Input ndarray dtype={gray_image.dtype}")

        gpu_src = ensure_gpu(gray_image)
        print(f"[remove_lines] After ensure_gpu: type={gpu_src.type()}, CV_8U={cv2.CV_8U}")

        # S'assurer que l'image est en uint8 pour threshold (type <= 4 requis par CUDA)
        if gpu_src.type() != cv2.CV_8U:
            print(f"[remove_lines] Converting from {gpu_src.type()} to CV_8U")
            gpu_src = gpu_src.convertTo(cv2.CV_8U)
            print(f"[remove_lines] After conversion: type={gpu_src.type()}")

        print(f"[remove_lines] About to call threshold with type={gpu_src.type()}")
        print(f"[remove_lines] GpuMat info: size={gpu_src.size()}, empty={gpu_src.empty()}, channels={gpu_src.channels()}")
        print(f"[remove_lines] GpuMat depth={gpu_src.depth()}, type={gpu_src.type()}")

        # Test: télécharger et re-uploader pour forcer une GpuMat valide
        print(f"[remove_lines] Forcing download/upload cycle...")
        temp_cpu = gpu_src.download()
        print(f"[remove_lines] Downloaded to CPU: dtype={temp_cpu.dtype}, shape={temp_cpu.shape}")
        gpu_src_clean = cv2.cuda_GpuMat()
        gpu_src_clean.upload(temp_cpu)
        print(f"[remove_lines] Re-uploaded to GPU: type={gpu_src_clean.type()}, depth={gpu_src_clean.depth()}")

        # Binarisation OTSU sur GPU
        _, gpu_thresh = cv2.cuda.threshold(gpu_src_clean, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
class OptimizerGUI:
    ...
    def evaluate_pipeline(self, params):
        ...
        print(f"[evaluate_pipeline] USE_CUDA = {USE_CUDA}")
